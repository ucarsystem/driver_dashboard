...
# Streamlit UI 구성
st.set_page_config(page_title="운전자 대시보드", layout="wide")
st.title("🚗 운전자별 개인인 대시보드")
...
if st.button("조회하기") and company_input and user_id_input and user_name_input and year_input and month_input:
    ...
    if os.path.exists(file_path):
        df_final = pd.read_excel(file_path, sheet_name="매크로(운전자리스트)")
        ...
        if not filtered.empty:
            ...
            st.warning("🚫 해당 운전자를 찾을 수 없습니다. 이름/ID/운수사를 다시 확인해주세요.")
